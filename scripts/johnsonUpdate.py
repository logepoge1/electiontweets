...
last_id = query("SELECT twitter_id FROM GovGaryJohnson ORDER BY twitter_id DESC LIMIT 1")

if last_id is None:
	user_timeline = api.get_user_timeline(screen_name="GovGaryJohnson", count=200)
else:
	last_id = int(last_id[0]) + 1
	user_timeline = api.get_user_timeline(screen_name="GovGaryJohnson", count=200, since_id=last_id)
